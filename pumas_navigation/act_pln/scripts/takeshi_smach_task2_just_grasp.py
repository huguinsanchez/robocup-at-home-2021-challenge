...
class Scan_table(smach.State):
    ...
    def execute(self,userdata):
        ...
        move_base_goal(2.2,4.2,goal_yaw)
        head_val = head.get_current_joint_values()
        head_val[0] = np.deg2rad(0)
        head_val[1] = np.deg2rad(-45)        
...
if __name__== '__main__':
    ...
    with sm:
        ...
        smach.StateMachine.add("INITIAL",       Initial(),      transitions = {'failed':'INITIAL',      'succ':'SCAN_TABLE',    'tries':'INITIAL'}) 
        ...
        smach.StateMachine.add('POST_FLOOR',    Post_floor(),   transitions = {'failed':'GRASP_FLOOR',  'succ': 'END',       'tries':'SCAN_TABLE'}) 
